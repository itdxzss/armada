...
让群权限设置继续遵循“本地选择管理员 → 协议设置 → 同账号实时 metadata 确认”，并保证回读字段完整、只声明经过协议确认的可写能力。
...
- 群权限设置不再在请求主链路同步读取 metadata，也不再因读取延迟、字段缺失或状态暂不一致而把协议成功改写为失败。
- 仍由本地快照选择在线、正常且在群内的管理员/群主，调用 Web 或 Android 对应设置接口；协议设置成功后立即返回，随后排队 `METADATA_CHANGED` 刷新任务。
- 前端保留已提交的开关状态，后台轮询 metadata 刷新任务；刷新完成后再以快照更新页面，失败时提示手动刷新，不回滚已提交请求。
