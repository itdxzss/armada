...
| E-H2-04 | 新建可选普通按钮、卡片按钮、单图文；历史双图文只回显；单图文有兼容性告警 | `task-0vbZUOmq.js:1738-1765, 1937-1968` | 原样实现 |
...
|---|---|---|---|
| 消息类型 | `messageType` | 必填；新建/复制可选 1/3/4，编辑锁定；2 仅历史回显 | `hyperlink_task_content.message_type` |
...
取消不修改主表单；清空只清可编辑条件，不去除三个固定条件。空筛选含义是全部符合固定条件的账号。
...
| UI 能力 | 接口 | 要求 |
|---|---|---|
| 模式、价格、余额、协议数、固定运行常量 | `GET /api/hyperlink-tasks/create-context` | 打开时并行加载 |
...
| 分组/协议/渠道/国家 | 复用各资源 option 接口 | 只返回当前租户可见项 |
...
纯新建且 `enabled=true` 的提交流程：先通过校验和账号试算，再请求 H3 `POST /quote`，打开 520px 最后核对弹框，
展示当前余额（账户+赠送）、服务端预计冻结、超级模式徽标、任务名、数据包名称/未使用数、消息类型、匹配账号数、
推广链接或按钮 URL、深度追踪状态及多国家价格明细。按钮为“返回修改 / 确认无误提交”；确认按钮倒计时 7 秒，
倒计时结束才能提交。报价过期则原弹框内刷新报价并重新倒计时。

`enabled=false` 新建不报价，直接保存。复制和编辑不弹竞品不存在的第二次 7 秒框，H3 在同一保存事务前重新校验
...
- 组件：三种可选消息及历史双图回显、实时预览、素材选择/上传/更换/清空、按钮添加/删除/追踪。
- 账号筛选：全部字段双向映射、包含/排除互斥、区间错误、取消不落值、清空、旧请求不覆盖新请求。
