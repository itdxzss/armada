...
> 实施状态（2026-08-03）：已按“当前成员实时查询 + HistorySync/实时通知采集退群事实 + 实时 `add` 采集进群事实”分支完成代码开发，尚未部署。现有协议没有按群主动查询完整退群历史的接口，因此未实现文中待验证的主动查询分支；历史完整性仍须在第二套测试环境用真实账号验证。
...
- 复用 WhatsApp 已有的 `w:gp2 add` 通知，不新增 WhatsApp 原始事件或主动查询接口。
- 在线且仍在群内的 Armada Android 账号只承担“观察并转发”作用；被加入的普通成员不需要是 Armada 控端账号。
- Android 通过内部事件 `account.group_participant_joined` 上报成员 JID、可解析手机号、事件时间和源事件 ID。
- Armada 仅保存每个“租户 + 群 + 成员”的最近一次进群事实，不保存当前群成员快照。
- 全量成员明细和按国家导出共用该进群时间；群统计中的“累计成功进群号码数量”按任务加入群后的去重手机号统计。
- 只能统计部署后实际收到的 `add` 事件；WhatsApp 未重放的部署前历史进群时间继续留空，禁止用 Armada 账号关系时间补值。
