...
三个接口的失败统一为实际 **4xx/5xx**，body 仅 `{ "message": "可读原因" }`，不解析管理员响应信封。响应均 `Cache-Control: no-store`。
...
查询成功、用户选组、网络发送完成都不能触发官方登出。只有上传 200 且合法 WAITING_LOGOUT 响应后才提供“退出并交接”。用户暂不退出时控端不得上线。查询失败、上传失败、超时或响应不合法时保留登录状态并提示重试。
...
- 手机端：获取分组、选择交互、三个字段上传、受理响应校验、失败保留登录、成功后退出并确认交接。
