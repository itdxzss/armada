...
10. 暂停期间新检测到的群不创建第 0 轮等待记录，恢复后不补第 0 轮；该群仍可正常参加后续普通轮次。
...
暂停期间检测到的新群因为任务不是 `SENDING`，不创建 WAITING，也不在恢复后补第 0 轮。
...
| 新状态影响统计 | WAITING 不进入现有明细终态查询；转为 SKIPPED 后只计入跳过数 |
...
| AC-008 | 暂停期间检测到新群 | 不产生第 0 轮记录，恢复后不补第 0 轮 | Service 测试 |
