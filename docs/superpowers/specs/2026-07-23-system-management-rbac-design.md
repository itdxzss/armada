...
现有业务菜单是初始化事实来源，但不原样复制旧的前端分组。本次按业务归属重新整理目录，页面原有 `route_path` 和 `component_path` 保持不变：
...
旧前端的“推广管理/数据中心”中间目录和“（二期）”阶段性文案不进入初始化数据。买号上量现有按钮权限作为 `B` 节点初始化，包括模板可见性、模板备注、渠道新增/编辑/检测/停用、统计编辑和导出。系统管理按钮权限按本期实际页面操作初始化。其他页面目前只有页面级 `perm_key`，不凭空补造未在前端和后端使用的按钮权限。
