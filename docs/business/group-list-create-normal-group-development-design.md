...
3. 建群前，建群人与本群每个初始成员必须同时在线并完成两次定向联系人保存：建群人保存成员、成员保存建群人；两次调用均返回成功即视为本业务的“双向好友准备成功”，任何一次失败，该群不得进入建群调用。禁止为此查询双方完整通讯录。
...
1. **双向好友判定已闭环**：不查询管理员或成员账号的完整通讯录，也不新增全量好友关系扫描接口。对本群每一对“建群人—成员”只执行两次定向 `ContactPort.save`；双方账号均在线且两次调用均成功（当前 `void` 接口表现为正常完成、未抛异常），即满足本功能建群前置条件。该口径表达的是 Armada 的业务通过条件，不额外承诺 WhatsApp 存在可查询的“真实好友”状态。
...
- 每个成员严格完成 creator→member、member→creator 后才调用 create；
- 双方账号在线且两次定向 `saveContact` 均成功时直接判定好友准备通过，不调用通讯录列表查询接口；
- 任一双向联系人步骤失败，不调用 create；
