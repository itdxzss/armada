...
- 日期：2026-09-12
- 状态：已部署第一套 test1，并通过登录态页面数量验收；见 deployment.md。
...
- 后续已完成 test1 发布及登录态浏览器只读验收：全部分组 950、未分组 712，筛选未分组后的列表总数也是 712；空分组显示 0。未执行数据写入测试或真库直连。

## 发布与回滚

- 后端 `fcce0cee`、前端 `2c27ce28` 已推送 `1.0.3-snapshot` 并发布到第一套 test1。
- 合入前按 `expert-reviewer` 复核，未发现阻断项。重跑后端 31 项及前端 8 项聚焦测试、tsc/vue-tsc 与变更文件静态检查均通过；既有 5 项群组回归基线失败不属于本次改动。
