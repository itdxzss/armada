...
## 群详情关闭重开一致性（2026-08-18）

- 根因：权限协议成功后只更新了前端内存，详情重开从 `wa_group_profile` 读取时仍得到旧值；限时消息还保留同步 metadata 回读与自动刷新路径。
- 修复：限时消息和五项权限在协议成功后，只将本次提交字段同步写入 `wa_group_profile`，然后返回成功；协议失败不写本地快照。
- 明确不做：不在写请求内读取 metadata，不创建 metadata 刷新任务，不增加定时轮询。用户显式点击“刷新群信息”或收到后续群变化事实时，仍可覆盖本地提交快照。
