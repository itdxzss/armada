...
- 状态：前后端实现与本地自动化验证完成；等待 V157 合入，尚未部署
...
- `db/migration`：新增 V158 素材元数据、标签、引用索引和 RBAC。
...
- 显式 `FOR UPDATE` 和跨表引用统计必须显式传入可信租户 ID，并跳过自动租户改写。
- 列表查询不读取 `MEDIUMBLOB content`；标签和引用数按当前页批量查询，禁止 N+1。
- 上传只接受可真实解码的 JPG/JPEG，单张不超过 500KB；每批最多 100 张并串行上传。
- 当前分支没有 V157 任务表，V158 不猜测未落地结构；任务表合入后需补任务引用统计和写入锁。
- V158 禁止先于 V157 部署；若目标环境已有任务能力，补齐任务引用保护是发布阻塞项。
...
- 未部署时：回退本次 Java、XML、前端、文档和 V158 文件即可。
- 已部署时：先回退依赖素材 API 的前后端并完成数据备份，再按 `rollback.sql` 删除菜单、标签表、索引和新增元数据列。
- Flyway 已登记 V158 的环境还需按部署规范处理 schema history，禁止只删除表或直接篡改共享库历史。

## 后续

- 在 V157 对应任务模型合入后，为真实任务快照引用补聚合统计、删除保护、绑定行锁和索引回归。
