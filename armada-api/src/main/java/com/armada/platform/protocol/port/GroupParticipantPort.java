...
public interface GroupParticipantPort {
    ...
    /**
     * 使用同一个在线协议账号批量变更 WhatsApp 群成员角色或移除成员。
     *
     * <p>本端口只接受 {@link GroupParticipantAction} 定义的升管理员、降管理员和移除动作，
     * 不包含添加成员。一次请求中的全部目标 JID 必须由同一账号在同一群内执行，调用方负责
     * 事先完成执行账号选择和群主保护等业务校验。</p>
     *
