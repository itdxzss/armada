...
public interface MarketingTemplateService {
     ...
     * 分页搜索营销模板,供列表页展示。
     *
     * <p>按 ID 精准、模板名模糊、文本类型与消息类型组合筛选(多条件为且关系),
     ...
     * 新增营销模板,供后续创建营销任务时引用。
     *
     * <p>落库前校验:模板名非空且 owner 范围内不重复、内容必填、文本选填、消息类型合法;
     * 按钮规则——仅按钮超链可配 1~3 个按钮,普通超链和图文内容不可配按钮。</p>
     *
     * @param dto 模板配置
     * @return 创建后的模板
     ...
     * 复制营销模板。
     *
     * <p>基于本人源模板生成一条新模板,名称追加「副本」后缀;若「副本」名已存在则报冲突,
     * 提示先重命名后再复制。共享/转移上线前不复制其他 owner 的模板。</p>
     *
     * @param id 源模板 ID
     * @return 复制生成的新模板
     ...
     *
     * <p>跨域约束：被删模板若被营销任务引用，需将仍占用账号的非终态任务按异常终止置为已完成
     * 并释放账号；已有终态任务保留历史状态。</p>
     *
