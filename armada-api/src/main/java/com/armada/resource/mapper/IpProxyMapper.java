...
public interface IpProxyMapper {
     ...
     *
     * <p>匹配失败时原账号绑定，或尚未成功重检的空闲代理，避免旧事件误伤后续新绑定。
     * 失败检测快照由 entity 承载，失败次数在 SQL 中原子递增。</p>
     */
    int markFailedProxyUnavailable(@Param("failure") IpProxyFailureContext failure,
                                   @Param("usingStatus") int usingStatus,
                                   @Param("idleStatus") int idleStatus,
                                   @Param("successCheckStatus") int successCheckStatus,
