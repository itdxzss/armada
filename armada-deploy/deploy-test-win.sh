...
source_lf "${SCRIPT_DIR}/lib/common.sh"
# shellcheck source=lib/artifact.sh
source_lf "${SCRIPT_DIR}/lib/artifact.sh"
# shellcheck source=lib/armada.sh
source_lf "${SCRIPT_DIR}/lib/armada.sh"
# shellcheck source=lib/protocol.sh
source_lf "${SCRIPT_DIR}/lib/protocol.sh"
# shellcheck source=lib/zhuan.sh
source_lf "${SCRIPT_DIR}/lib/zhuan.sh"
# shellcheck source=lib/deep-check.sh
source_lf "${SCRIPT_DIR}/lib/deep-check.sh"
armada_init_colors
...
resolve_wsl_ssh_key() {
  ...
  local role_name="$2"
  local secure_key_dir secure_key

  # DrvFs 上的 PEM 权限通常会被 OpenSSH 判定为过宽；复制到 WSL ext4 后再使用。
  if ! is_wsl || [[ "${configured_key}" != /mnt/* ]]; then
    printf '%s\n' "${configured_key}"
    return 0
  fi

  [ -f "${configured_key}" ] || die "找不到 ${role_name} SSH 私钥: ${configured_key}"
  command -v install >/dev/null 2>&1 || die "自动复制 Windows 私钥需要 install 命令"

  secure_key_dir="${ARMADA_WSL_KEY_DIR:-${HOME}/.ssh/armada-deploy}"
  mkdir -p "${secure_key_dir}"
  chmod 700 "${secure_key_dir}"
  secure_key="${secure_key_dir}/${ENV_ID}-${role_name}.pem"

  # 只有源私钥发生变化时才覆盖，避免无意义地刷新文件时间。
  if [ ! -f "${secure_key}" ] || ! cmp -s "${configured_key}" "${secure_key}"; then
    install -m 600 "${configured_key}" "${secure_key}"
  else
...
prepare_windows_ssh_keys() {
  if [ "${BUILD_BE}" = 1 ] || [ "${BUILD_FE}" = 1 ] || [ "${CHECK_ONLY}" = 1 ]; then
    SSH_KEY="$(resolve_wsl_ssh_key "${SSH_KEY}" armada)"
  fi
  if [ "${BUILD_PROTOCOL}" = 1 ] || [ "${CHECK_ONLY}" = 1 ]; then
    PROTOCOL_SSH_KEY="$(resolve_wsl_ssh_key "${PROTOCOL_SSH_KEY}" protocol)"
...
prepare_windows_deploy_assets() {
  local source_dir="${DEPLOY_ASSET_DIR}"
  local asset
  local assets=(
    backend.prebuilt.Dockerfile
    nginx.prebuilt.Dockerfile
    render-platform-config.sh
    nginx.conf
    stale-chunk-reload.js
    docker-compose.rds.yml
    .env.example
  )

  WINDOWS_ASSET_DIR="$(mktemp -d "${TMPDIR:-/tmp}/armada-deploy-win-assets.XXXXXX")"
  for asset in "${assets[@]}"; do
    [ -f "${source_dir}/${asset}" ] || die "缺少 ${source_dir}/${asset}"
    cp "${source_dir}/${asset}" "${WINDOWS_ASSET_DIR}/${asset}"
...
cleanup_branch_worktree() {
  ...
  case "${WINDOWS_ASSET_DIR:-}" in
    "${TMPDIR:-/tmp}"/armada-deploy-win-assets.*)
      rm -rf -- "${WINDOWS_ASSET_DIR}"
      ;;
  esac
}
...
armada_find_jdk17() {
  ...
  # WSL 必须使用 Linux JDK；Windows 的 java.exe 不能作为 Linux Maven 的 JAVA_HOME。
...
configure_windows_maven() {
  local configured_home="${ARMADA_MAVEN_HOME:-${MAVEN_HOME:-}}"
  local settings_file="${ARMADA_MAVEN_SETTINGS:-}"
  local local_repo="${ARMADA_MAVEN_LOCAL_REPO:-}"

  if [ -n "${configured_home}" ]; then
    [ -x "${configured_home}/bin/mvn" ] \
      || die "ARMADA_MAVEN_HOME 必须指向 WSL/Linux Maven，且包含可执行的 bin/mvn: ${configured_home}"
    WINDOWS_MAVEN_BIN="${configured_home}/bin/mvn"
  else
    # type -P 只查找外部命令，避免解析到下方同名包装函数后发生递归调用。
    WINDOWS_MAVEN_BIN="$(type -P mvn 2>/dev/null || true)"
  fi
  [ -n "${WINDOWS_MAVEN_BIN}" ] || die "构建后端需要 WSL/Linux Maven；可设置 ARMADA_MAVEN_HOME"

  if [ -n "${settings_file}" ]; then
    [ -f "${settings_file}" ] || die "找不到 Maven settings.xml: ${settings_file}"
    WINDOWS_MAVEN_ARGS+=(-s "${settings_file}")
  fi
  if [ -n "${local_repo}" ]; then
    mkdir -p "${local_repo}"
    WINDOWS_MAVEN_ARGS+=("-Dmaven.repo.local=${local_repo}")
  fi
}

mvn() {
  "${WINDOWS_MAVEN_BIN}" "${WINDOWS_MAVEN_ARGS[@]}" "$@"
}
...
usage() {
  cat <<EOF
deploy-test-win.sh - 部署 armada API + wheel-saas-pure-web + Baileys/Zhuan 协议层到测试服。

用法:
  ./armada-deploy/deploy-test-win.sh [options]
  ./armada-deploy/deploy-test-win.sh          只显示部署指引,不执行部署。
...
可覆盖的环境变量:
  ...
  ARMADA_WSL_KEY_DIR      Windows 盘私钥自动复制到此 WSL 安全目录
  ARMADA_MAVEN_HOME       WSL/Linux Maven 安装目录
  ARMADA_MAVEN_SETTINGS   Maven settings.xml，可复用 Windows 盘上的镜像配置
  ARMADA_MAVEN_LOCAL_REPO Maven 本地仓库目录，可复用 Windows 盘上的仓库
...
你要部署什么?
  ...
  后端 + 前端:
    ./armada-deploy/deploy-test-win.sh --all -y

  后端 + 前端 + Baileys 协议层 + Zhuan 协议:
    ./armada-deploy/deploy-test-win.sh --full -y

  只部署后端:
    ./armada-deploy/deploy-test-win.sh --be -y

  只部署前端/nginx:
    ./armada-deploy/deploy-test-win.sh --fe -y

  只部署协议层:
    ./armada-deploy/deploy-test-win.sh --protocol -y

  只部署 Zhuan 协议:
    ./armada-deploy/deploy-test-win.sh --zhuan -y

  只看部署计划,不真正执行:
    ./armada-deploy/deploy-test-win.sh --dry-run

  只读深度检查:
    ./armada-deploy/deploy-test-win.sh --env perf2 --check

  部署指定 armada 分支:
    ./armada-deploy/deploy-test-win.sh --all --branch main -y
    ./armada-deploy/deploy-test-win.sh --be --branch feature/group-import -y
    ARMADA_DEPLOY_BRANCH=main ./armada-deploy/deploy-test-win.sh --be -y
...
--branch 说明:
  ...
  5. 如果只想确认会部署哪个分支,先加 --dry-run:
     ./armada-deploy/deploy-test-win.sh --be --branch main --dry-run
