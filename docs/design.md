...
- **安全网**:不是"老测试绿 + 端点 diff=0",而是 **TDD 新测试 + H2 内存数据库测试 + 业务验收**。
  - 硬约束:数据库测试必须在 `mvn test` 中使用 test scope 的 H2、真实 Mapper XML、MyBatis-Plus 插件和 Spring 事务真跑；禁止因缺 `.env` 静默跳过。真库 DbTest 仅作可选补充验证。
...
- [x] **#8** 迁移方式 → §11:模型 B 全栈纵切、按业务逐块重建;每块走 梳理→设计→TDD→验收;安全网 = 新测试 + H2 内存数据库测试 + 业务验收;基础设施先行
