...
日期：2026-09-10。以下主体记录本地验证；后续 test1 真实部署和业务验收见 [acceptance-20260910.md](acceptance-20260910.md)。
...
- 用户指定 test1 后，已部署前后端与 V186/V187；隔离候选在本地提交，未推送。
- 迁移重编号后 23 个聚焦测试通过；AccountProfileMapper 列名歧义修复后，22 个聚焦测试及六个生产 upsert 的 MySQL 8.4 临时表验证通过。
- 四个页面/抽屉改为 v-perms，移除 E2E 的人工 meta.auths 后，5 条浏览器测试再次通过；相关 lint/格式检查通过。
- 普群任务 147/148 和进群任务 74 成功；营销任务 2 两群各四条，8 条协议发送成功。任务 1 最终 7 成功、1 条注入故障失败，测试群权限已恢复。
- 已读取真实四表 information_schema，并运行 gen_datamodel.py 生成本次数据模型增量。完整证据及接收回执边界见验收记录。
