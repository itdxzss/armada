...
def monitor_event(node, second, *, lag=0, produced=0, consumed=0, valid=True):
    value = {
        "schemaVersion": 1,
        ...
        "resource": {
            ...
            "valid": valid,
            "errorClass": None if valid else "docker_stats",
        },
    }
    if node == "zhuan":
        value["kafka"] = {
...
class FakeTaskAPI:
    def __init__(self, inventories, calls):
    ...
    def resume_snapshot_once(self, snapshot, concurrency):
        ...
        self.resume_calls += 1
        now = datetime(2026, 7, 25, 2, 0, 2, tzinfo=UTC)
