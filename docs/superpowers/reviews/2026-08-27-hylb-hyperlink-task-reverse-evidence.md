...
- remediation: recipient 作为一位收信人的唯一发送与首触归因事实，配合账号累计投影和首访时间趋势查询；敏感归因加权限、审计和保留期，不丢竞品字段、不臆造逐次点击页面。
...
- 「每账号最大发送数」tooltip 明确是单账号在**本任务内**允许成功发送的总上限，达到后自动停发，
  0 表示不限制（`task-0vbZUOmq.js:2672-2707`）。
...
6. 账号类型：个人/商业（字段 `account_type`）。
7. 导入方式：六段参数/全参数。
8. 类型：分身设备 `web5`/主设备 `native6`（字段 `wid_type`）。
9. 设备类型（字段 `platform`）：安卓个人、安卓商业（主设备/分身）、苹果个人、苹果商业（主设备/分身）；
   六个实际值见 `account-platform-BYq5VTkz.js:1-80`。
10. 账号性质（字段 `source`）：买量、自登、买入、转入、群扫码（枚举在 `581-619`）。
11. 允许拉群：全部/允许/不允许。
12. 好友数最小/最大。
13. 留存天数最小/最大，允许小数。
...
- 该列表以一位收信人一行展示；前端读取的行字段没有轮次号、尝试序号或尝试历史入口（`3008-3262`）。
  这能证明竞品页面需要的是 recipient 级最终流水，不能单凭静态前端反推出后台另有“一人多轮/多次尝试”模型。
...
- PV 列 tooltip 明确是“按首次访问所在时间段近似归集”：同一访客的全部访问次数归入首访桶，
  不能按每次真实 `visit_at` 分桶（`visit-trend-tab-CVJUtu9z.js:486-501`）。
...
| 任务/recipient | 尚未实现 | 新增任务领域；同任务同号码一行，recipient 承载唯一发送命令和最终状态 |
...
| 短链和访问归因 | 未实现 | recipient 保存首次 IP/UA/设备、累计 UV/PV 与首末时间，权限和保留期控制 |
| 五个详情读模型 | 未实现 | 由 recipient、任务账号用量、账号快照与账号累计投影聚合；访问趋势直接按 recipient 首访时间查询 |
...
- 收信人流水按 recipient 一人一行展示，前端没有轮次/尝试历史字段或入口。
- 生命周期 API 无删除。
...
1. 即时模式的数据包领取批次、子任务拆分和协议超时恢复边界。
2. 预发布模式「数据包发完」与新导入号码的代次关系。
3. 周期文案虽称“反复执行”，但静态前端不能证明是否重复营销同一号码，也不能证明后台轮次唯一键和结算口径。
4. `default_sub_task_num=50` 的竞品后端真实语义。
5. 价格是否按国家/号码/成功发送计费，以及停止后的冻结释放公式。
6. 封号原因统计的账号失效判定窗口。

这些是**竞品证据边界**，不再是 Armada 悬空的实现选择：Armada 已按页面“一位收信人一行”的可见模型冻结为
同任务同号码最多发送一次，round 只分配剩余 recipient，超时只重放同一 command，任务只预约一次余额；
封号窗口也在详细设计和 `docs/business/hyperlink-marketing-data-model.md` §4 冻结。价格公式仍是接入真实账务
提供方的上线硬依赖。
后续拿到真实 API 样例只做兼容性验证，不能把未证实的内部行为冒充前端反推事实。

预计落地率不属于后端未知算法：竞品列表前端明确计算
`min(99, deliveryRate * 100 + 20)%`（`task-0vbZUOmq.js:5213-5263`），Armada 原样实现。
