...
**「用隐私可见性反推双向好友」这条路已评估并明确放弃**（2026-08-29 决定，别再重新纠结）：

WhatsApp 协议里没有好友关系概念。唯一可能的推断路径是——「关于/头像/最后在线」的默认隐私是
「我的联系人」，能看到对方的「关于」就说明我在对方通讯录里。`fetchStatus(...jids)` 支持
USync 批量，成本本身可控。
...
它只在对方用默认隐私时才准，而这个比例没有实测数据。这个数会被拿去做
「双向好友 ≥ N」筛选——**基于系统性偏差的数字筛号，用户以为筛出了高质量号，
实际筛的是「隐私设置宽松的号」**，比没有这个筛选更糟。

另外：**绝对不要用 `profilePictureUrl` 逐个拉头像**（单个接口，5000 联系人 = 5000 次 IQ，
批量拉头像是典型封号触发器）。

将来真要做，前置条件是：① R1/R2 先验完；② 先拿几十个已知关系的号做真机对照量准确率，
低于八成不上；③ 落列叫 `is_status_visible` 而不是 `is_mutual`，UI 标「疑似双向（估算）」。
