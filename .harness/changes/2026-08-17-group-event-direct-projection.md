...
- [x] 上线按基线分档查询：基线已建走轻量、未建走完整；dirty 与显式对账仍强制完整
      （`b709993`，armada-protocol。该改动非本任务所写，是工作区里已完成未提交的成果，
      为保持后续 diff 干净代为单独落一刀，提交前已验证 tsc 干净、相关 93 用例全绿）
- [x] Web 轻量群清单补齐简介与四个群设置，与安卓口径对齐（`661bae1`，armada-protocol）
