...
- [x] Phase 1：账号、账号分组、账号导入批次完整隔离，覆盖列表、统计、详情、创建、修改、删除、批量操作、导出与上线诊断（代码完成，未部署）。
- [x] Phase 2：群链接运营句柄、WS 链接分组、群文件夹、链接导入/批处理、营销模板/图片与标准拉群头像已完成代码和迁移（未部署）；群 JID/协议事实继续保持租户级 canonical。
- [x] Phase 3：进群、标准拉群、普通营销、拉群营销、建群营销、新建普群和群批处理任务及其结果/导出/统计已完成 owner 继承链（未部署）。
- [x] Phase 4：推广渠道、文件、直接 SQL、Scheduler、Kafka、Outbox 和恢复链路完成收口；直接 JDBC、协议回调、异步导出、核心调度/恢复及群实时协议操作均已覆盖 owner 边界（未部署）。
- [x] Phase 5：前端保存可信当前身份能力、隔离跨用户本地状态，并修正 401/403/404/409 错误处理；代码、类型检查和生产构建完成，未部署。
...
- commit / 环境 / 部署后验证结果: 未部署；任何测试/生产环境部署前另行确认。
