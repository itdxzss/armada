...
- 状态：V120 六表、V121 profile 健康列和 V122 健康检测人数列均已部署 test1，人工回填及已定位漏写数据的定向补齐完成；baseline/first-post 硬门禁通过。旧表仍决定列表、营销和全部业务结果，影子 Mapper 尚未接入 Service，未切换读取。固定数据库水位覆盖 11,391 行的核对已证明健康、最终成员数、分组、邀请、备注、头像和建群时间对齐；管理员、群主和可用账号的差异均已分类为新模型中更新的成员事实，不是缺表、断链或漏迁。仍保留 87 条未解析邀请和 1 条不可见 Unicode subject/名称归一化差异，且尚未完成 test1 全部筛选、排序和分页的影子查询验收，因此不能记为全部 parity 通过
