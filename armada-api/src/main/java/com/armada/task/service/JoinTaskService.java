...
public interface JoinTaskService {
     ...
     * 建进群任务。
     *
     * <p>保存前先校验链接文本:非空行必须是以 https://chat.whatsapp.com/ 开头的群邀请链接。校验通过后,
     * 按方式一(每链接固定账号数、账号跨链接连续轮询)或方式二(每账号进前 K 条链接)生成 PENDING 行。
     * 任务建为 DRAFT,total/pending 取实际 PENDING 行数,引擎计数(executed/success/failed)恒 0。
     * 本方法只落库,不触发协议层执行。</p>
     *
     * @param req 建任务入参(任务名、分组/账号快照、链接文本、分配方式与间隔/重试配置)
     * @return 建成后的任务列表行视图(含 id、计数、状态)
     * @throws BusinessException 任务名称为空或群链接格式错误时抛 {@link ErrorCode#VALIDATION}
     */
     ...
     * @return 编辑后的任务详情
     * @throws BusinessException 任务不存在抛 {@link ErrorCode#NOT_FOUND};已执行、任务名为空或群链接格式错误抛 {@link ErrorCode#VALIDATION}
     */
