...
- 联系人准备失败项重试时，先查冻结成员的实时协议运行态：恢复在线则继续复用；
  仍不可执行则从原成员分组重新选择数据库正常且协议运行态在线的账号。替换成员时
...
### 2026-08-09 成员分组实时在线筛选修复

- 新建任务不再只依赖 `account_state.login_state=ONLINE` 快照；在冻结管理员和成员前，通过
  `AccountRuntimeStatusPort` 分别查询 Web owner/会话与 Android `WaApp` 实时运行态，只选择当前
  明确 `ONLINE` 的账号。
- `PREPARING_CONTACTS` 失败重试时，冻结成员已恢复实时在线则原号重试；否则在原成员
  分组内选择其他实时在线账号，不得与建群账号或已冻结成员重复。
- 替换使用失败阶段与联系人状态条件更新，重置旧错误和两个 commandId；旧账号的迟到
  回执无法命中新一轮 fencing。
- 重试使用两阶段提交：协议 HTTP 运行态探测在事务和 item 行锁之外完成；短事务内重新锁定
  item、校验成员快照未变，并按预期旧成员账号条件替换，避免 HTTP 超时长时间阻塞 Kafka
  结果消费或并发重试。
