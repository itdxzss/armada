...
- 状态: 已合并 `1.0.3-snapshot` 最新代码并部署第一套测试环境；接口与运行期隔离验收通过，页面已完成账号列表冒烟，但跨路由 Playwright 验收受认证态丢失阻塞
...
- 当前用户私有业务域的代码隔离已完成并在 test1 通过接口与运行期验收：账号/分组/导入、群运营句柄/分组/文件夹/导入/批处理、营销模板与全部任务类型、推广渠道/统计/CAPI、导出、协议回调和恢复链均已收口；IP、canonical 群协议事实、国家等按需求继续租户/平台共享。页面端目前只有账号列表完成实操冒烟，跨路由 Playwright 受新标签认证态无法持续保持阻塞；生产发布、历史 owner 分配以及共享/转移仍不在本轮范围。
