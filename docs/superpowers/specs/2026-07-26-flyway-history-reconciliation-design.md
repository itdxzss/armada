...
`flyway repair` 的前提下，与数据库已经执行的 V061-V079 完全兼容。
...
| V076 | remove obsolete group management menu | 271914839 |
| V077 | account desired login state | -1202454221 |
| V078 | pull task and channel stats | 1996711734 |
| V079 | optimize IP country stats | -2121604516 |

当前 `1.0.2-snapshot` 只包含：

- `V061__group_pull_marketing.sql`，其 Flyway 校验和为 `-168361012`，实际对应数据库 V070；
- `V062__account_desired_login_state.sql`，其 Flyway 校验和为 `-1202454221`，实际对应数据库 V077。

因此问题不是 SQL 内容本身，而是分支合并后丢失了 V061-V079 的既有历史，并复用了已执行版本号。

## 方案比较

### 方案一：代码侧恢复真实迁移历史（采用）

从第一套环境当前可正常启动的旧 JAR 中按原始字节恢复 V061-V079；用其已经通过
Flyway 校验的内容作为历史事实。账号期望登录状态恢复到其真实历史版本 V077。

优点：不篡改数据库历史；所有已执行校验和保持一致；新环境也能按完整顺序建库。
代价：需要恢复 19 个历史脚本，并增加契约测试防止再次丢失或改写。

### 方案二：执行 `flyway repair`（否决）

把数据库 V061/V062 校验和改成当前代码值。这样会把已经执行的推广迁移伪装成拉群和账号状态迁移，
同时遗漏 V063-V079，数据库事实与代码事实永久分叉。
...
1. `db/migration` 中恢复旧 JAR 的 V061-V079，文件名和内容逐字节保持一致。
2. 删除当前错误编号的 `V061__group_pull_marketing.sql` 和
   ...
   - 账号期望登录状态由恢复后的 `V077__account_desired_login_state.sql` 承载。
3. 不新增手工 DDL，不修改 `flyway_schema_history`。部署时 V061-V079 全部只做校验，
   不产生新的待执行迁移。
4. 更新拉群营销迁移测试路径为 V070。
5. 恢复 Flyway 全局版本唯一性测试，并增加 V061-V079 文件名、描述和校验和契约测试。
...
1. V061-V079 每个版本恰好一个文件，文件名与预期映射一致；
2. V061-V079 的 Flyway 校验和与第一套测试库逐项一致；
3. V070 保持拉群营销校验和 `-168361012`；
4. V077 保持账号期望登录状态校验和 `-1202454221`；
5. Maven 打包后的 JAR 确实包含完整 V061-V079，而不是只检查源码目录；
6. 部署前再次只读查询第一套测试库历史，部署后确认容器不重启、日志无 Flyway 错误、接口返回预期未登录响应。

## 回滚

- 代码回滚：恢复部署前旧后端制品。本次不执行新迁移，因此没有新增数据库回滚动作。
- 数据回滚：禁止手工删除历史记录或修改校验和。
