...
public class FeedTaskMessageCommandFactory {
    ...
    public FeedTaskMessageCommandFactory(MarketingTemplateFileService fileService) {
        this.fileService = fileService;
    }

    /** 从任务组合一条 Status 内容。 */
    public ComposedFeedStatus composeContent(FeedTask task) {
        MarketingTemplateFileContent file = task.getLinkPreviewImageFileId() == null
                ? null : fileService.content(task.getLinkPreviewImageFileId());
        byte[] imageBytes = file == null ? null : file.content();
        String mimetype = file == null ? null : file.contentType();
