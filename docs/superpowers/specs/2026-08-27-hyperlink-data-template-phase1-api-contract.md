...
本轮并行开发以远端最高 `V140` 为基线，预留如下，禁止两个 Agent 自行抢号：

| 版本 | 所有者 | 内容 |
|---|---|---|
| `V141` | 数据包后端 | 数据包四表和索引 |
| `V142` | 模板后端 | `hyperlink_template` 和索引 |
| `V143` | 后续集成分支 | 菜单与 RBAC，四个开发分支不得创建 |

每个后端 Agent 提交前仍须 fetch 并检查 `origin/1.0.3-snapshot`。如果远端已经出现同号迁移，必须停止
并报告集成负责人统一重新编号，不能在自己的分支静默换号后让合同失真。
